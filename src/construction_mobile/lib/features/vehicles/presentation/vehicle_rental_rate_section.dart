...
import '../data/models/vehicle_rental_rate.dart';
import '../data/vehicle_rental_rate_repository.dart';
import 'vehicles_controller.dart';

final _vehicleRentalRatesProvider = FutureProvider.autoDispose
    .family<List<VehicleRentalRate>, String>((ref, vehicleId) async {
  final page = await ref
      .watch(vehicleRentalRateRepositoryProvider)
      .fetch(vehicleId: vehicleId, pageSize: 10);
  return page.items;
});

/// The rent/lease rate history for a vehicle — shown only once the vehicle
/// itself is marked `Rented`. `ownershipType` alone only says a vehicle is
/// rented; this is what actually puts a rate (and thus a monthly cost) on
/// record, mirroring the desktop admin panel's "Renta / lizing" card.
class VehicleRentalRateSection extends ConsumerWidget {
  const VehicleRentalRateSection({super.key, required this.vehicleId});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final rates = ref.watch(_vehicleRentalRatesProvider(vehicleId));
        ...
        Padding(
          ...
          child: Row(
            children: [
              ...
              if (canRecord)
                TextButton.icon(
                  onPressed: () => showVehicleRentalRateFormSheet(
                    context,
                    ref,
                    vehicleId: vehicleId,
                  ),
                ...
                children: [
                  for (final rate in items)
                    ListTile(
                      ...
                      onTap: canRecord
                          ? () => showVehicleRentalRateFormSheet(
                                context,
                                ref,
                                vehicleId: vehicleId,
                                existing: rate,
...
Future<void> _deleteRate(
  ...
  try {
    await ref.read(vehicleRentalRateRepositoryProvider).remove(rate.id);
    ref.invalidate(_vehicleRentalRatesProvider(rate.vehicleId));
    ref.invalidate(vehicleDetailProvider(rate.vehicleId));
...
Future<void> showVehicleRentalRateFormSheet(
  BuildContext context,
  WidgetRef ref, {
  required String vehicleId,
  VehicleRentalRate? existing,
}) {
  return showModalBottomSheet<void>(
    context: context,
    isScrollControlled: true,
    builder: (_) => _RentalRateFormSheet(vehicleId: vehicleId, existing: existing),
  );
}

class _RentalRateFormSheet extends ConsumerStatefulWidget {
  const _RentalRateFormSheet({required this.vehicleId, this.existing});

  final String vehicleId;
  final VehicleRentalRate? existing;
...
class _RentalRateFormSheetState extends ConsumerState<_RentalRateFormSheet> {
  ...
  Future<void> _submit() async {
    ...
    final repository = ref.read(vehicleRentalRateRepositoryProvider);
    ...
    try {
      ...
      ref.invalidate(_vehicleRentalRatesProvider(widget.vehicleId));
