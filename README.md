...
| Module | Endpoints |
|---|---|
| Material deliveries | `POST /api/material-movements` (a delivery needs an invoice number; `supplier` and the purchase `unitPrice` are recorded with it), `GET /api/material-movements/suppliers` (names already used, newest first), `POST /api/material-movements/import/preview` and `/import` (a .csv or .xlsx delivery list, one row per material, columns found by heading; unknown materials with a unit are created; rows already recorded are skipped), `GET /api/materials/{id}/pricing` (last and average purchase price, spending roles only). Creating a material with starting stock records it as a delivery (with invoice) or as an unpriced opening balance (without) |
| Worker housing | `GET/POST /api/accommodations` (name, type, city, floor, rooms, beds, area, landlord, contract, deposit, utilities, active), `GET/POST /api/accommodations/{id}/stays` and `PUT/DELETE /api/accommodations/stays/{id}` (who lives where, with dates; a person is in one place at a time), charges through `/api/accommodation-rates` of three kinds (`Monthly` for the unit, `DailyPerPerson`, `OneOff`; monthly and daily each keep their own rate chain), `GET /api/accommodations/{id}/costs?from=&to=` (what a period cost, split by day between the people who lived there and by project, with empty days shown as vacancy) |
